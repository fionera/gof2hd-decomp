...
- template: a FREE container helper (ArrayAdd|Remove|Release|ReleaseClasses|SetLength|AddCached|RemoveAll|Set<T>) the original emits out-of-line. Ensure the template is defined in Array.h and add ONE explicit instantiation in the shared Array TU (src/engine/core/Array.cpp) for the original's exact T — from normal C++, never asm. DO NOT hand-instantiate Array<T> MEMBER functions (Array<T>::resize/clear/push_back/erase/Array/~Array/operator=) — those are inline in the generic Array<T> template (Array.h) and the compiler emits them wherever the type is used; per-type "template void Array<X>::resize(...)" lines are byte-match clutter and are excluded from the goal. If a template entry is an Array<T> member, treat it as already handled (defer/no-op).
