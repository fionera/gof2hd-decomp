...
extern "C" int SpaceLounge_OnTouchBegin(SpaceLounge *self, int x, int y)
{
    ...
    if (UC(self, 0x1b) != 0 || UC(self, 0x19) != 0) {
        ChoiceWindow_OnTouchBegin(self->f_8, x, y);
        return 0;
    }

    if (UC(self, 0x34) != 0) {
        StarMap_OnTouchBegin(self->f_4, x, y);
        return 0;
    }

    if (UC(self, 0x1c) != 0) {
        ListItemWindow_OnTouchBegin(self->f_c, x, y);
        Layout_OnTouchBegin(*(void **)SpaceLounge_layout_begin, x, y);
        ScrollTouchWindow_OnTouchBegin(self->f_60, x, y);
        return 0;
    }
    ...
    if (mode == 0) {
        ...
        TouchButton_OnTouchBegin(*(void **)buttons->f_4, x, y);
    } else if (mode == 2) {
        unsigned i = 0;
