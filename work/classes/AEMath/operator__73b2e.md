...

{
  ...
  if ((!in_ZR) && (in_CY)) {
    coprocessor_storelong(7,in_cr15,in_r12);
    if (in_NG) {
      *unaff_r5 = unaff_r8 & unaff_r4 << 0xc;
    }
    coprocessor_moveto(7,3,3,unaff_pc,in_cr8,in_cr6);
  ...
  pVVar1 = param_1;
  if (in_NG) {
    pVVar1 = param_1 + -(unaff_r4 >> 0x19 | unaff_r4 << 7);
    *(undefined4 *)param_1 = unaff_lr;
  }
  coprocessor_storelong(7,in_cr15,unaff_r8);
  if (!in_CY) {
    *(Vector **)pVVar1 = param_2;
  }
                    /* WARNING: Bad instruction - Truncating control flow here */
  halt_baddata();
}
