...
void LodMeshMerger::update()
{
    ...
    if (this->f_3c != 0) {
        ...
        for (int j = 0; j < rows; j++) {
            if (*((int8_t *)this->f_30 + j) != 0 &&
                *((int8_t *)this->f_34 + j) != 0) {
                signed char lod = *((int8_t *)this->f_2c + j);
                void *src = ((void **)this->f_24)[rows * lod + j];
        ...
        for (int j = 0; sumIdx >= 0x10000 && j < rows; j++) {
            if (*((int8_t *)this->f_30 + j) != 0 &&
                *((int8_t *)this->f_34 + j) != 0) {
                signed char lod = *((int8_t *)this->f_2c + j);
                if (lod < this->f_38 - 1) {
                    void *prev = ((void **)this->f_24)[rows * lod + j];
                    setLod(j, (signed char)(lod + 1));
                    rows = this->f_0;
                    signed char nlod = *((int8_t *)this->f_2c + j);
                    void *cur = ((void **)this->f_24)[rows * nlod + j];
