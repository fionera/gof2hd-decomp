    ...
    if (self->f_3c != 0) {
        if (F<uint8_t>(self, 0xae) != 0) {
            for (int i = 0xc; i != 0x11; i++)
                TouchButton_OnTouchMove(G<void *>(G<void *>(self->f_24, 4), i * 4), touch);
            TouchButton_OnTouchMove(G<void *>(G<void *>(self->f_24, 4), 0x44), touch);
        } else if (self->f_b0 != 0) {
            for (int i = 0x12; i != 0x17; i++)
                TouchButton_OnTouchMove(G<void *>(G<void *>(self->f_24, 4), i * 4), touch);
        }
        ChoiceWindow_OnTouchMove(self->f_20, touch);
        return 0;
    }

    if (self->f_58 == 1) {
        ListItemWindow_OnTouchMove(self->f_18, touch);
        return 0;
    }

    if (G<int>(layout, 0xc) < coord && coord < *g_hw_screenWidth - G<int>(layout, 0x10)) {
        int dy = coord - self->f_b8;
        self->f_c0 = dy;
        self->f_c4 = 1.0f;
        self->f_b4 = self->f_b4 + dy;
        self->f_b8 = coord;
