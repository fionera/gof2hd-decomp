...
extern "C" void HangarWindow_initialize(HangarWindow *self)
{
    ...
    Array<void *> *tabArr = (Array<void *> *)self->f_4;
    ...
    *(unsigned int *)*g_hw_imageCountSlot = tabArr->length;
    ...
    G<void *>(G<void *>(self->f_24, 4), 0) = e0;
    ...
    G<void *>(G<void *>(self->f_24, 4), 0xc) = e3;
    ...
    G<void *>(G<void *>(self->f_24, 4), 0x10) = e4;

    void *e5 = operator_new(200);
    TouchButton_ctor_text2(e5, GameText_getText(*g_hw_helpTextId), 7, 0, 0, self->f_110, 0x11);
    G<void *>(G<void *>(self->f_24, 4), 0x14) = e5;
    void *e6 = operator_new(200);
    TouchButton_ctor_text2(e6, GameText_getText(*g_hw_helpTextId), 7, 0, 0, self->f_110, 0x11);
    G<void *>(G<void *>(self->f_24, 4), 0x18) = e6;
    void *e7 = operator_new(200);
    TouchButton_ctor_text(e7, GameText_getText(*g_hw_helpTextId), 7, 0, 0, 0x11);
    G<void *>(G<void *>(self->f_24, 4), 0x1c) = e7;

    {
        ...
        G<void *>(G<void *>(self->f_24, 4), 0x20) = e8;
        AEString_dtor(&lbl);
    }
    {
        String12 lbl;
        ...
        G<void *>(G<void *>(self->f_24, 4), 0x24) = e9;
        AEString_dtor(&lbl);
    }
    {
        ...
        G<void *>(G<void *>(self->f_24, 4), 0x28) = e10;
    }
    {
        String12 credits;
        ...
        G<void *>(G<void *>(self->f_24, 4), 0x2c) = e11;
    ...
    for (int slot = 0xc; (unsigned int)(slot - 0xc) < 5; slot++) {
        ...
        G<void *>(G<void *>(self->f_24, 4), slot * 4) = btn;
        AEString_dtor(&lbl);
        TouchButton_setVisible(G<void *>(G<void *>(self->f_24, 4), visIdx * 4), false);
        row++;
    }
    {
        String12 lbl;
        ...
        if (listMode == 0)
            ...
            TouchButton_ctor_text(btn, &lbl, 10, 0, 0, 1);
        G<void *>(G<void *>(self->f_24, 4), 0x44) = btn;
        AEString_dtor(&lbl);
    ...
    PaintCanvas_Image2DCreate(*g_hw_canvas, 0x233f, (unsigned int *)((char *)self + 0x38));
    {
        ...
        G<void *>(G<void *>(self->f_24, 4), 0x5c) = btn;
        AEString_dtor(&lbl);
    }
    ...
    for (int i = 0x12; (unsigned int)(i - 0x12) < 5; i++) {
        ...
        G<void *>(G<void *>(self->f_24, 4), i * 4) = btn;
        TouchButton_setVisible(G<void *>(G<void *>(self->f_24, 4), i * 4), false);
    }

    self->f_120 = TouchButton_getWidth(0);
    int h = TouchButton_getHeight(G<void *>(G<void *>(self->f_24, 4), 0x30));
    self->f_124 = h;
    self->f_128 = (int)((float)(-self->f_120) * g_hw_posScale);
    self->f_12c = (int)((float)(-h) * g_hw_posScale);
    ...
    self->f_dc = PaintCanvas_GetImage2DWidth(*g_hw_canvas);
    self->f_e0 = PaintCanvas_GetImage2DHeight(*g_hw_canvas);

    // Recompute best cargo prices from known equipment when not in a black market.
    if (self->f_10 != 0 && Status_inBlackMarketSystem() == 0 &&
        F<uint8_t>(self, 0x11d) == 0) {
        unsigned int *equip = (unsigned int *)Ship_getEquipment(Status_getShip());
        Array<void *> *cargo = (Array<void *> *)self->f_10;
    ...
    HangarList_setCurrentTab(self->f_14, *g_hw_specialModeFlag != 0);
    HangarWindow_refreshCurrentContentHeight(self);

    self->f_a8 = Ship_getCurrentLoad(Status_getShip());
    void *lay2 = *g_hw_layout;
    self->f_d8 = ((*g_hw_screenHeight - G<int>(lay2, 0x10) - G<int>(lay2, 0xc)) -
