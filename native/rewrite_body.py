...
# fix the header include, and pull in cross-class headers. Writes native/src/<Class>.cpp and prints
...
CM=json.load(open("native/classmap.json")); CLASSES=set(CM)
...
def rewrite(cls):
    ...
    if re.search(r'^#include "[^"]+\.h"\n', txt, re.M):
        ...
        txt=inc+txt
    open(f"native/src/{cls}.cpp","w").write(txt)
    rem_F=len(re.findall(r'\b[FG]\s*<',txt)); rem_cast=len(re.findall(r'\(char ?\*\)',txt))
    return f"{cls}: -> native/src/{cls}.cpp | refs={sorted(refs)} | UNCONVERTED: {left[0]} (F/G left={rem_F}, casts left={rem_cast})"
