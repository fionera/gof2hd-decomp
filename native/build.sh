...
# Pass 1: compile every native/src/*.cpp -> native/build/obj/*.o (64-bit macOS, no byte-matching).
# Reports per-file pass/fail + the aggregate error-category histogram for whatever still fails.
set -u
cd "$(dirname "$0")/.."
OBJ=native/build/obj; mkdir -p "$OBJ"; LOG=native/build/errs.txt; : >"$LOG"
CXX="clang++ -std=gnu++14 -fpermissive -w -ferror-limit=0 -Inative/include"
ok=0; fail=0; faillist=""
for f in native/src/*.cpp; do
  [ -f "$f" ] || continue
  c=$(basename "$f" .cpp)
