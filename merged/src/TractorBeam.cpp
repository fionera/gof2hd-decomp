...
{
    void *crate = F<void *>(self, 0xc);
    void *radarCrate = F<void *>(radar, 0x1c);
    ...
    if (crate == 0) {
        ...
        if (PlayerEgo_isInTurretMode(player) == 0) {
            if (KIPlayer_cargoAvailable(radarCrate) == 0) {
                ...
                if (F<void *>(radarCrate, 0x78) == 0)
                    KIPlayer_createCrate(radarCrate, 0);
                F<void *>(self, 0xc) = radarCrate;
                u32(self, 0x18) = (uint32_t)Player_getHitpoints(F<void *>(radarCrate, 0x4));
                u8(self, 0x10) = 1;
            }
            return;
        }
        crate = F<void *>(self, 0xc);
        if (crate == 0)
            goto detach;
    }

    // Drop the beam if the crate has lost its body or the carrier is gone.
    if (F<void *>(crate, 0x78) == 0 ||
        (u8(crate, 0x3c) == 0 && Player_isActive(crate) == 0)) {
    detach:
        F<void *>(radar, 0x8) = 0;
        F<void *>(radar, 0x1c) = 0;
        F<void *>(self, 0xc) = 0;
    ...
    Vector pos;
    AEGeometry_getPosition(&pos, F<void *>(self, 0x14));
    ...
    AEGeometry_setScaling(F<void *>(self, 0x14), offset.x, offset.y, offset.z);

    // Aim the beam at the crate and place its tail at the player position.
    void *geo = F<void *>(self, 0x14);
    ...
    if (dist > gCaptureDistance) {
        ...
        void *crateGeo = F<void *>(F<void *>(self, 0xc), 0x78);
        Vector pull = AbyssEngine::AEMath::operator-(n, *(Vector *)self);
        AEGeometry_translate(crateGeo, &pull);

        if (KIPlayer_isDead(F<void *>(self, 0xc)) != 0 ||
            KIPlayer_isDying(F<void *>(self, 0xc)) != 0) {
            void *c = F<void *>(self, 0xc);
            if (u8(c, 0x40) == 0 && u8(c, 0x41) == 0) {
        ...
        KIPlayer_captureCrate(F<void *>(self, 0xc), hud);
        F<void *>(self, 0xc) = 0;
        u8(self, 0x10) = 0;
        F<void *>(radar, 0x8) = 0;
        F<void *>(radar, 0x1c) = 0;
