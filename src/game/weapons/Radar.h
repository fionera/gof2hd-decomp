...
public:
    void *level; // +0x00
    // +0x04: cockpit lock target; the docking-controller flavor of this object
    // (used by PlayerEgo::approachDockingPoint / dockToDockingPoint, which treat
    // the pointer as a KIPlayer-derived docking handler) stores the active dock
    // target here. Same 4-byte pointer slot, two names via the union alias.
    union {
        KIPlayer *lockedEnemy; // +0x04
        void *dockTargetPtr; // +0x04
    };

    // +0x08: cockpit unused slot / docking nav-point pointer (same memory).
    union {
        void *field_0x8; // +0x08
        void *dockNavPtr; // +0x08
    };
    ...
    int field_0x58; // +0x58 (real C++ offset 0x44; ends 0x48)
    // ---- docking-controller reservation bytes ----------------------------
    // PlayerEgo's docking code (approachDockingPoint @ 000afcb8,
    // dockToDockingPoint @ 000af9f4) accesses this object at +0x70 and +0x8c:
    //   if (*(char*)(p+0x70)) *(char*)(p+0x8c) = 1;   // has a slot reservation,
    //                                                 // mark it dirty/handled
    // These two single bytes must live at their exact C++ offsets, so explicit
    // padding pins them down (the cockpit Radar leaves this span unused).
    uint8_t pad_0x48_to_0x70[0x70 - 0x48]; // fill .. +0x6f
    uint8_t hasReservation; // +0x70  (read as char)
    uint8_t pad_0x71_to_0x8c[0x8c - 0x71]; // fill .. +0x8b
    uint8_t reservationDirty; // +0x8c  (written 1)
    uint8_t pad_0x8d_to_0x90[0x90 - 0x8d]; // fill .. +0x8f (align next int)
    int screenX; // (cockpit overlay block, packed)
    int screenY; // +0x100
