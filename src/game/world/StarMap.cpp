...
void StarMap::depart(bool jump)
{
    ...
    if (this->jumpMapModeA != 0) {
        ...
        *g_StarMap_depart_store0_a = 0;
        ...
        if (target->equals((Station *)((Status *)(status))->getStation()) == 0) {
...
StarMap::StarMap(bool jumpMapMode, Mission *mission, bool param3, int param4)
{
    ...
    AEGeometry *root = new AEGeometry((PaintCanvas*)(long)(*g_StarMap_ctor_canvas));
    ...
    for (uint32_t i = 0; i < systemsGeom->size(); i++) {
        ...
        AEGeometry *geom = new AEGeometry(image, (PaintCanvas*)(long)(*g_StarMap_ctor_canvas), false);
    ...
    if (((Status *)(*g_StarMap_ctor_status))->getCurrentCampaignMission() > 0x1f &&
        ...
        AEGeometry *marker = new AEGeometry((uint16_t)0x4262, (PaintCanvas*)(long)(*g_StarMap_ctor_canvas), false);
...
void StarMap::initStarSystem()
{
    ...
    ((AbyssEngine::AERandom *)(*g_StarMap_system_random))->setSeed((long long)system->getIndex() * 1000);

    Array<AEGeometry *> *stationGeoms = new Array<AEGeometry *>();
    this->stationGeoms = stationGeoms;
    stationGeoms->resize(count + 1);

    AEGeometry *root = new AEGeometry((PaintCanvas*)(long)(*g_StarMap_system_canvas));
    ...
    for (uint32_t i = 1; i < stationGeoms->size(); i++) {
        ...
        AEGeometry *geom = new AEGeometry((uint16_t)(tex + 0x4704), (PaintCanvas*)(long)(*g_StarMap_system_canvas), false);
        ...
        if (this->markerGeom != 0 && this->selectedSystem == field<int32_t>(*g_StarMap_system_status, 0x7c) &&
            Station_getIndex(this->stations->data()[stationIndex]) == field<int32_t>(*g_StarMap_system_status, 0x80)) {
    ...
    for (uint32_t i = 0; i < rings->size(); i++) {
        AEGeometry *ring = new AEGeometry((uint16_t)0x1a7b, (PaintCanvas*)(long)(*g_StarMap_system_canvas), false);
        (*rings)[i] = ring;
    ...
    this->pitch = 0.0f;
    this->selectedStation = -1;
    ((PaintCanvas*)(long)(*g_StarMap_system_canvas))->Image2DCreate((unsigned short)((uint16_t)(0x4500 + system->getRace())), &this->systemNameImage);
    ...
    AEGeometry *planet = new AEGeometry((uint16_t)0x1a70, (PaintCanvas*)(long)(*g_StarMap_system_canvas), false);
