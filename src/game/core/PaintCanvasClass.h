...
namespace AbyssEngine {
    ...
    public:
        ...
        // +0x0  This word is read two ways in the binary: as a 1-byte init /
        // gravity-rotation flag, and (in the PE turret / ship paths in
        // PlayerEgo.cpp) as the full 32-bit value passed to TransformGetTransform /
        // handleShip as a "self handle". Both names live at the exact same offset 0
        // via a union, so neither access needs a raw cast and the layout (next
        // member field_0x4 at +0x4) is unchanged.
        union {
            bool gravityRotationEnabled; // +0x0    init / gravity-rotation flag (low byte)
            unsigned int selfHandle; // +0x0    self handle (full 32-bit word)
        };
        ...
        void DrawImage2D(unsigned int index, int x, int y, int w, int h, unsigned char anchor, unsigned char align,
        ...
        void DrawRegion2D(unsigned int index, int srcX, int srcY, int srcW, int srcH, float angle, int dstX, int dstY,
                          int dstW, int dstH);
        ...
        void GetLineArray(unsigned int font, const AbyssEngine::String &str, int maxWidth,
                          ::Array<AbyssEngine::String *> *out);
        ...
        void TextureCreate(unsigned short id, void (*loadCallback)(AbyssEngine::Image *, void *), void *userData,
                           unsigned int &out, bool flag);
        ...
        void SetMatForGlow(AbyssEngine::Material *material);
