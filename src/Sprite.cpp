#include "gof2/Sprite.h"



// ---- setPosition_d0e52.cpp ----
void Sprite::setPosition(int x, int y)
{
    this->field_0x10 = x;
    this->field_0x14 = y;
}

// ---- setRefPixelPosition_d0e4c.cpp ----
void Sprite::setRefPixelPosition(int x, int y)
{
    this->field_0x10 = x;
    this->field_0x14 = y;
}

// ---- _Sprite_d0e30.cpp ----
Sprite::~Sprite()
{
    void *frames = this->field_0x0;
    if (frames != 0) {
        operator delete(frames);
    }
    this->field_0x0 = 0;
}

// ---- defineReferencePixel_d0e46.cpp ----
void Sprite::defineReferencePixel(int x, int y)
{
    this->field_0x8 = x;
    this->field_0xc = y;
}

// ---- nextFrame_d1018.cpp ----
void Sprite::nextFrame()
{
    return setFrame(this->field_0x38 + 1);
}

// ---- draw_d0e68.cpp ----
__attribute__((visibility("hidden"))) extern void **g_Sprite_draw_image_canvas;
__attribute__((visibility("hidden"))) extern void **g_Sprite_draw_region_canvas;
extern "C" void PaintCanvas_DrawRegion2DForDraw(void *canvas, uint32_t image, int srcX, int srcY, int w, int h,
                                                long long zeroPair, int flags, int x, int y);

void Sprite::draw(float scaleX, float scaleY)
{
    uint32_t *frames = this->field_0x0;

    if (frames == 0) {
        uint32_t image = this->field_0x4;
        int refX = this->field_0x8;
        int posX = this->field_0x10;
        int posY = this->field_0x14;
        int w = this->field_0x18;
        int h = this->field_0x1c;
        int y = posY - refX;
        posY = 0;
        int x = posX - refX;
        PaintCanvas_DrawRegion2DForDraw(*g_Sprite_draw_region_canvas, image,
                                        this->field_0x28, this->field_0x2c, w, h,
                                        (long long)(uint32_t)posY, posY, x, y);
        return;
    }

    void **holder = g_Sprite_draw_image_canvas;
    int frame = this->field_0x38;
    int refX = this->field_0x8;
    int posX = this->field_0x10;
    void *holderValue = *holder;
    uint32_t image = frames[frame];
    int x = posX - refX;
    void *canvas = holderValue;

    if (scaleX == 1.0f || scaleY == 1.0f) {
        return PaintCanvas_DrawImage2D(canvas, image, x, this->field_0x14 - refX);
    }

    float frameHeight = (float)this->field_0x1c;
    float frameWidth = (float)this->field_0x18;
    int y = this->field_0x14 - refX;
    int scaledHeight = (int)(frameHeight * scaleY);
    int scaledWidth = (int)(frameWidth * scaleX);
    int drawY = (int)((float)y - ((scaleY - 1.0f) * frameHeight) * 0.5f);
    int drawX = (int)((float)x - ((scaleX - 1.0f) * frameWidth) * 0.5f);

    PaintCanvas_DrawImage2DScaled(canvas, image, drawX, drawY, scaledWidth, scaledHeight, 0x11, 0x11, 0);
}

// ---- Sprite_d0d18.cpp ----
Sprite::Sprite(uint32_t image, int frameWidth, int frameHeight)
{
    void **holder = g_Sprite_canvas;

    this->field_0x0 = 0;
    this->field_0x4 = image;
    this->field_0x18 = frameWidth;
    this->field_0x1c = frameHeight;

    void *canvas = *holder;
    this->field_0x20 = PaintCanvas_GetImage2DWidth(canvas, image);
    int imageHeight = PaintCanvas_GetImage2DHeight(*holder, this->field_0x4);

    int rows = imageHeight / this->field_0x1c;
    int columns = this->field_0x20 / this->field_0x18;

    this->field_0x8 = 0;
    this->field_0xc = 0;
    this->field_0x10 = 0;
    this->field_0x14 = 0;

    this->field_0x24 = imageHeight;
    this->field_0x30 = columns;
    this->field_0x34 = rows;
    this->field_0x3c = rows * columns;
    setFrame(0);
}

// ---- prevFrame_d1020.cpp ----
void Sprite::prevFrame()
{
    return setFrame(this->field_0x38 - 1);
}

// ---- drawRegion_d0f8c.cpp ----
void Sprite::drawRegion(int srcX, int srcY, int w, int h)
{
    uint32_t *frames = this->field_0x0;
    void **holder = g_Sprite_canvas;
    void *canvas = *holder;

    if (frames != 0) {
        int refX = this->field_0x8;
        int posX = this->field_0x10;
        int posY = this->field_0x14;
        uint32_t image = frames[this->field_0x38];
        int y = posY + srcY - refX;
        int x = posX + srcX - refX;
        PaintCanvas_DrawRegion2D(canvas, image, srcX, srcY, w, h, 0.0f, 0, 0, x, y);
        return;
    }

    uint32_t image = this->field_0x4;
    int refX = this->field_0x8;
    int posX = this->field_0x10;
    int posY = this->field_0x14;
    int frameX = this->field_0x28;
    int frameY = this->field_0x2c;
    int y = posY + srcY - refX;
    int x = posX + srcX - refX;
    srcX += frameX;
    srcY += frameY;
    PaintCanvas_DrawRegion2D(canvas, image, srcX, srcY, w, h, 0.0f, 0, 0, x, y);
}

// ---- Sprite_d0dd0.cpp ----
Sprite::Sprite(uint32_t *frames, int frameCount, int frameWidth, int frameHeight)
{
    void **holder = g_Sprite_canvas;

    this->field_0x0 = frames;
    this->field_0x4 = (uint32_t)-1;
    this->field_0x18 = frameWidth;
    this->field_0x1c = frameHeight;

    void *canvas = *holder;
    this->field_0x20 = PaintCanvas_GetImage2DWidth(canvas, *frames);
    int imageHeight = PaintCanvas_GetImage2DHeight(*holder, *this->field_0x0);

    this->field_0x8 = 0;
    this->field_0xc = 0;
    this->field_0x10 = 0;
    this->field_0x14 = 0;

    this->field_0x24 = imageHeight;
    this->field_0x30 = 1;
    this->field_0x34 = 1;
    this->field_0x3c = frameCount;
    setFrame(0);
}

// ---- setFrame_d0d88.cpp ----
void Sprite::setFrame(int frame)
{
    if (frame < 0) {
        frame = -frame;
    }

    this->field_0x38 = frame;
    if (frame >= this->field_0x3c) {
        frame = frame % this->field_0x3c;
        this->field_0x38 = frame;
    }

    int frameWidth = this->field_0x18;
    int frameHeight = this->field_0x1c;
    int columns = this->field_0x30;
    int row = frame / columns;

    int column = frame - row * columns;
    int frameY = row * frameHeight;
    int frameX = column * frameWidth;

    this->field_0x28 = frameX;
    this->field_0x2c = frameY;
}
