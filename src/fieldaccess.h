...
// GOF2 byte-offset accessor helpers — read/write a typed lvalue at a raw byte offset of an object.
// The decompiled code references many struct members by hard offset before the field has a name;
// these helpers give a single definition for that idiom (they were once duplicated per header).
// Prefer named struct fields where the layout is known; these are the bridge for the rest.
#include <cstdint>
